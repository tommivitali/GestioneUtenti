<?php
  ...
  function elenco($filename) { 
    ...
    if (($handle = @fopen($filename, "r")) !== FALSE) {
      while (($linecsv = fgetcsv($handle, 1000, ";")) !== FALSE) {
        ...
        echo '<td class="pass">' . $linecsv[1] . '</td>';
  ...
  function aggiungi_utente($filename) {
    $libero = true;
    if (($handle = @fopen($filename, "r+")) !== FALSE) {
      while (($linecsv = fgetcsv($handle, 1000, ";")) !== FALSE && $libero) {
        if($linecsv[0] == $_POST['user']) {
          $libero = false;
        }
      }

      if($libero) {
        fwrite($handle, $_POST['user'] . ";" . $_POST['pass'] . ";" . $_POST['nome'] . ";" . $_POST['tipo'] . "\n");
        echo "<p>L'utente " . $_POST['user'] . " è stato aggiunto correttamente.</p>";
      }
      else alert("Il nome utente non è disponibile!");
